...
df = query_db(ACTIVITIES_PER_WEEK_QUERY, conn)
...
with tab1:
    ...
    for col in df.columns[1:]:
        bars = plt.bar(
            df["year"].astype(str), df[col], bottom=bottom, label=col, edgecolor="black"
        )
        # Add labels
        for bar in bars:
            if bar.get_xy()[1] == 0:
...
with tab2:
    ...
    bottom = None
    for col in df.columns[1:]:
        bars = plt.bar(df["year"], df[col], bottom=bottom, label=col, edgecolor="black")
        # Add labels
        for bar in bars:
            if bar.get_xy()[1] == 0:
                textcolor = "white"
    ...
    plt.title(
        "Percentage of rest days during training windows", loc="left", fontweight="bold"
    )
...
with tab3:
    ...
    df = query_db(STREAKS_QUERY, conn)
    ...
    type_offset = {"Active Streak": -0.1, "Raw Streak": 0.1}

    palette = {"Active Streak": "#0A2540", "Raw Streak": "#5C9EAD"}
